...
class ProcRunner extends EventEmitter {
    constructor(options) {
        super();
        options = options || {};
        this._cpuCount = options._cpuCount || concurrency;
        this.startedCount = 0;
        this.errCount = 0;
        this.killedCount = 0;
        this.successCount = 0;
        ...
        if (options.printStatus) require('./status-handler')(this);
...
function makeProcClass(options) {
    ...
    class Proc extends EventEmitter {
        ...
        _onData(chunk) {
            ...
            const lastTwoLines = chunk.toString().split('\n').splice(-3);

            if (lastTwoLines.length <= 2)
                this._dataBuffer += lastTwoLines[0];
            else {
                this._lastDataLine = lastTwoLines[1];
                this._dataBuffer = lastTwoLines[2];
            }
            if (lastTwoLines.length === 2) {
                this._lastDataLine = this._dataBuffer;
                this._dataBuffer = lastTwoLines[1] || '';
            }
